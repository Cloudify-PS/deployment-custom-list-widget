    ...
    value: function render() {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(DataTable.Column, { label: 'Actions', width: '20%', show: !!buttons.length }),
...
Stage.defineWidget({
  ...
  initialConfiguration: [Stage.GenericConfig.POLLING_TIME_CONFIG(5), Stage.GenericConfig.PAGE_SIZE_CONFIG(), Stage.GenericConfig.SORT_COLUMN_CONFIG('created_at'), Stage.GenericConfig.SORT_ASCENDING_CONFIG(true), {
  ...
  fetchData: function fetchData(widget, toolbox, params) {
    ...
    return _promise2.default.all([deployments, executionsData]).then(function (data) {
      ...
      var outputPromises = formattedData.items.map(function (item) {
        return toolbox.getManager().doGet('/deployments/' + item.id + '/outputs');
      });
      return _promise2.default.all(outputPromises).then(function (outputs) {
        formattedData.items = formattedData.items.map(function (i, idx) {
